  ...
  async load () {
    ...
    const processSubSection = () => {
      if (subSectionBuffer.length) {
        this.subSections.push(new SubSection(this, subSectionBuffer.splice(0, subSectionBuffer.length)))
      }
    }
...
class SubSection {
  ...
  constructor (section, [header, ...entries]) {
    this.#section = section
    this.#entries = entries

    const key = header.text.replace(/\W/g, '')
    const existing = this.#section.subSections.filter(section => section.title.replace(/\W/g, '') === key)
